...
from .sdk import TencentSpeech, AliSpeech, XunfeiSpeech, atc, VITSClient, VolcSpeech
import requests
...
class AbstractTTS(object):
    ...
    @abstractmethod
    async def get_speech(self, phrase):
        pass

    @abstractmethod
    async def get_speech_ws(self, phrase, silent, speed_ratio, emotion, character_category, operation):
        pass
    ...
    @abstractmethod
    async def get_speech_http(self, phrase, silent, speed_ratio, emotion, character_category):
        pass

    @abstractmethod
    def stop_websocket_stream(cls):
        pass
...
class VolcTTS(AbstractTTS):
    ...
    def __init__(
        ...
        api_url_ws="wss://openspeech.bytedance.com/api/v1/tts/ws_binary",
        api_url_http="https://openspeech.bytedance.com/api/v1/tts/ws_binary",
        **args,
    ):
        super(self.__class__, self).__init__()
        self.engine = VolcSpeech.volcSpeech(appid, token, cluster, asr_cluster, voice_type, host, api_url_ws, api_url_http)
        self.appid, self.token, self.cluster, self.voice_type, self.host, self.api_url_ws, self.api_url_http = appid, token, cluster, voice_type, host, api_url_ws, api_url_http
