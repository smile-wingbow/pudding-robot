...
class WifiConfigCharacteristic(Characteristic):
    ...
    def ping(self, host='www.baidu.com', count=1):
        """
...
class LLMConfigCharacteristic(Characteristic):
    ...
    description = b"Config llm {'prompt', 'voice_type'}"
    ...
    def WriteValue(self, value, options):
        ...
        if len(self.buffer) >= self.expected_length:
            ...
            try:
                ...
                voice_type = config_data.get("voice_type")
                logger.info(f"prompt: {prompt}, voice_type: {voice_type}")
...
class WifiAdvertisement(Advertisement):
    def __init__(self, bus, index):
        ...
        self.add_service_uuid(WifiService.ESPRESSO_SVC_UUID)
...
def stop_mainloop():
    ...
    global mainloop
    if mainloop:
        logger.info("停止主循环，因为WiFi配置成功")
        mainloop.quit()
    return False  # 不重复调用

def signal_handler(sig, frame):
    """信号处理器，优雅退出"""
    logger.info(f"Signal {sig} received, stopping main loop")
    if mainloop:
        mainloop.quit()
