...
class ActiveListener(object):
    ...
    def listen(
        ...
        silentCount = 0
        recordingCount = 0
        dynamic_silent_threshold = silent_count_threshold  # 动态阈值，初始为原始值
        voice_detected = False  # 标志变量，记录是否检测到语音
        ...
        while self._running is True:
            ...
            if recordingCount > recording_timeout:
                ...
                if silentCount > dynamic_silent_threshold:
                    stopRecording = True
                else:
                ...
                if not voice_detected:  # 检测到语音后更新阈值
                    voice_detected = True
                    dynamic_silent_threshold = DYNAMIC_SILENT_THRESHOLD
                    logger.debug(f"Voice detected. New silent threshold: {dynamic_silent_threshold}")
            ...
            logger.info(f"recordingCount:{recordingCount}")
            logger.info(f"silentCount:{silentCount}")
